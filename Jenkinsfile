      ...
      stage('Docker打包推送') {
            steps {
                dir(path: "/${WORKSPACE}/codeway_admin_manage") {
                    ...
                    sh "docker tag codeway_admin_manage:${env.BUILD_ID} registry.cn-hangzhou.aliyuncs.com/codeway_me/codeway_admin_manage:${env.BUILD_ID}"
                    script {
                        sh "docker push registry.cn-hangzhou.aliyuncs.com/codeway_me/codeway_admin_manage:${env.BUILD_ID}"
                        echo "构建并推送到远程服务器成功--->"
                    }
                }
            }
